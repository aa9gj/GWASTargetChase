...
#' @param impc impc data
#' @param assocOT results from geneticAssocPrep function
#' @param l2gOT results from l2gPrep function
...
gwasFollowupMan <- function(sumStats, felGTF, pval = 0.00000005, ResultsPath = ".", impc = NULL, assocOT, l2gOT) {
  ...
  association_file <<- fread(assocOT)
  print("Reading OpenTargets locus to gene file")
  l2g_file <<- fread(l2gOT, sep = "\t", quote = "")
  ...
  for (i in seq_along(genes)) {
    results_g2d[[i]] <- gene2diseaseMan(genes[i])
  ...
  pb <- txtProgressBar(min = 0,      # Minimum value of the progress bar
                       ...
                       char = "=")   # Character used to create the bar
  for (i in seq_along(genes)) {
    results_l2g[[i]] <- locus2geneMan(genes[i])
    setTxtProgressBar(pb, i)
  }
